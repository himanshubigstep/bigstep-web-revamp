...
import { fetchAwsChallengesPage, fetchChallengesTypes } from '@/api-data/api'
...
const AwsTechnologiesPage = () => {
  const [challengescategories, setChallengesCategories] = useState(null)
  const [awsChallengesData, setAwsChallengesData] = useState<AwsChallengesPage | null>(null)
  const [loading, setLoading] = useState<boolean>(true);

  useEffect(() => {
    const fetchChallengesCategories = async () => {
      try {
        const response = await fetchChallengesTypes();
  ...
  useEffect(() => {
    const fetchAwsChallenges = async () => {
      try {
        const response = await fetchAwsChallengesPage();
  ...
  return (
    <div className='poppins'>
      <ChallengingFormSection
        challengesFormButtonText="Get Free AWS Assessment"
        challengescategories={challengescategories}
        challengesLabel="AWS Challenges"
...
export default AwsTechnologiesPage